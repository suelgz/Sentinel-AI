    ...
    try:
        ...
        with conn:
            ...
            for finding in findings:
                ...
                """, (
                    analysis_id,
                    safe_int(finding.get("threat_detected"), 1),
                    finding.get("threat_type", "Unknown"),
                    finding.get("severity", "Unknown"),
                    safe_float(finding.get("confidence"), 0.5),
                    safe_float(finding.get("rule_confidence"), 0.0),
                    ...
                    safe_int(finding.get("pattern_score"), 0),
                    finding.get("analysis_source", ""),
                ))

            return analysis_id
    except Exception as exc:
        print(f"Database save_analysis error: {exc}")
        return None
    finally:
        if conn:
            try:
